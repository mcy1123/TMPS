...
def call_guess_llm(
    ...
    for attempt in range(retries):
        try:
            ...
            if provider_key not in {"openai", "openrouter", "deepseek"}:
                raise ValueError(f"Unknown guess provider: {provider}")
            client = OpenAI(
                base_url=Utils.get_base_url(config, provider_key),
                api_key=Utils.get_api_key(config, provider_key),
            )
...
def guess_actions(
    ...
    prompt = Utils.build_guess_prompt(
        ...
        guess_prompt_template=config["prompts"]["guess"],
        num_guesses=config["game"]["num_guesses"],
        skill_enabled=config.get("guess", {}).get("skill_enabled", False),
        skill_prompt=config.get("guess", {}).get("skill_prompt", ""),
    )
    ...
    predictions = ChessActionCleaner.clean_actions(raw_output)[:num_guesses]
    return predictions, prediction_time, input_tokens, output_tokens, total_tokens
...
def process_trajectory_with_guesses(
    ...
    for step in step_info:
        ...
        passthrough_keys = [
            ...
            "gate_decision",
            "gate_reason",
            "prediction_hit_index",
            "prediction_hit_move",
            "used_speculation_index",
            "used_speculation_source_step",
        ]
        for key in passthrough_keys:
            if key in step_info[step]:
                new_step_info[step][key] = step_info[step][key]
        if "compact_observation" not in new_step_info[step]:
            new_step_info[step]["compact_observation"] = truncated_observation
        if "guess_prompt_mode" not in new_step_info[step]:
            guess_cfg = config.get("guess", {})
            new_step_info[step]["guess_prompt_mode"] = (
                guess_cfg.get("skill_mode", "off")
                if guess_cfg.get("skill_enabled", False) and guess_cfg.get("skill_prompt", "").strip()
                else "off"
            )
        if not prev_match:
            ...
            new_step_info[step]["ground_truth_in_guess"] = step_info[step]["current_move"] in predictions
            new_step_info[step]["ground_truth_guess_index"] = predictions.index(step_info[step]["current_move"]) if step_info[step]["current_move"] in predictions else -1
